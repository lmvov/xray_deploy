...
setup_cron_restart() {
  local restart_hour restart_min
  read -r -p "设置每天自动重启时间 - 小时 (0-23, 默认 4): " restart_hour
  restart_hour=${restart_hour:-4}
  read -r -p "设置每天自动重启时间 - 分钟 (0-59, 默认 0): " restart_min
  restart_min=${restart_min:-0}

  if ! [[ "$restart_hour" =~ ^[0-9]+$ ]] || ! [[ "$restart_min" =~ ^[0-9]+$ ]]; then
    echo "无效的时间格式，使用默认值 4:00"
    restart_hour=4
    restart_min=0
  fi

  local cron_job="${restart_min} ${restart_hour} * * * /usr/bin/systemctl restart xray >> /var/log/xray-restart.log 2>&1"
  ...
  echo -e "${GREEN}✅ 已设置每天 ${restart_hour}:${restart_min} 自动重启 Xray${RESET}"
}
