...
setup_cron_restart() {
  ...
  if echo "$existing" | grep -qF "vps restart"; then
    echo "定时重启已存在，已更新为新时间"
    (echo "$existing" | grep -v "vps restart"; echo "$cron_job") | crontab -
  else
...
cancel_cron_restart() {
  ...
  existing=$(crontab -l 2>/dev/null) || true
  if echo "$existing" | grep -qF "vps restart"; then
    echo "$existing" | grep -v "vps restart" | crontab -
...
manual_restart_xray() {
  echo "正在重启 VPS ..."
  date '+%Y-%m-%d %H:%M:%S' >> /var/log/vps-restart.log 2>/dev/null || true
  /sbin/reboot
}
...
main_menu() {
  while true; do
    ...
    case $choice in
      1) add_node; return_to_menu ;;
      2) show_nodes; return_to_menu ;;
      3) delete_node; return_to_menu ;;
      4) setup_cron_restart; return_to_menu ;;
      5) cancel_cron_restart; return_to_menu ;;
      6) manual_restart_xray; return_to_menu ;;
      7) show_restart_log; return_to_menu ;;
      0) echo "退出脚本"; exit 0 ;;
      *) echo -e "${RED}无效选项${RESET}"; return_to_menu ;;
    esac
  done
}
